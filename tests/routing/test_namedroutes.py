...
import hamcrest
import logging
from kavallerie.meta import APIView, RouteDefinition, RouteEndpoint
from kavallerie.routes import NamedRoutes


def test_add_duplicated_name(caplog):
    router = NamedRoutes()
    ...
    caplog.set_level(logging.WARNING, logger="kavallerie.routes")

    @router.register('/create', methods=['POST'], name="index")
    def obj_create(request):
        pass

    assert "Route 'index' already exists for path '/view': overriding." in caplog.text
